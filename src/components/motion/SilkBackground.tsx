...
function createProgram(gl: WebGLRenderingContext) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    const message = gl.getProgramInfoLog(program) || 'Erro desconhecido ao vincular shader.';
...
export function SilkBackground({ className }: SilkBackgroundProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.error('Falha ao iniciar o fundo Silk:', error);
    ...
    gl.uniform4f(uniforms.shape, 1.26, 0.28, 0.50, 0.00);
    gl.uniform4f(uniforms.surface, 2.40, 1.11, 0.00, 1.00);
    gl.uniform4f(uniforms.finish, 0.00, 0.00, 0.000, 0.05);
    gl.uniform4f(uniforms.transform, 1581.0, 0.00, 0.00, 0.0);
    ...
    const draw = (now: number) => {
      ...
      const seconds = reducedMotion.matches
        ? 0
        : elapsedBeforePause + (now - startedAt) / 1000;

      gl.useProgram(program);
      gl.uniform4f(uniforms.scene, canvas.width, canvas.height, seconds * 0.76, 4.0);
    ...
    const start = () => {
      ...
      if (!document.hidden && isVisible) frameId = requestAnimationFrame(draw);
    };

    const onVisibilityChange = () => {
      if (document.hidden) {
        pausedAt = performance.now();
        elapsedBeforePause += (pausedAt - startedAt) / 1000;
        cancelAnimationFrame(frameId);
      } else {
        start();
      }
  ...
  return (
    <div
      className={cn(
        'absolute inset-0 overflow-hidden bg-[#02010A]',
        className,
      )}
      aria-hidden="true"
    >
      <canvas
        ref={canvasRef}
        className="h-full w-full [display:block]"
      />
    </div>
  );
}
